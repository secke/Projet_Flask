...
def album(userId):
    ...
    for elem in albums:
        ...
        if not photos:
            ...
            return render_template('album.html', albums=albums, n=n, userId=userId, photos=photos)
    else:
        for elem in albums:
            print(userId)
...
def affiche_photo(userId):
    print(userId)
    photos = base.session.query(model.Photo.url, model.Photo.id).filter(model.Photo.albumId==userId).all()
    n=len(photos)
    if not photos:
        Photos()
    print(photos)
    return render_template('afiche_photo.html', photos=photos, albumId=userId, userId=userId, n=n)
...
def modifierPhoto(id):
    ...
    if request.method=='POST':
        ...
        if titre is None:
            ...
            # return redirect(url_for('photo'))
            return redirect(url_for('photo'))
    return render_template('modifierPhoto.html', photos=photo)

################ SUPPRIMER PHOTO ##################

@app.route('/supprimerPhoto/<int:id>/<int:userId>', methods=('POST','GET'))
def supprimerPhoto(id,userId):
    ...
    return redirect(url_for('affiche_photo', userId=userId))
